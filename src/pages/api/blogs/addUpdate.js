...
export default async function addUpdateBlog(req, res) {
  ...
  try {
    ...
    if (id && id.trim() !== "") {
      ...
      res
        .status(200)
        .json({ data: updateResult, message: "Blog Update Successfully!" });
    } else {
      ...
      const createResult = await blogsCollection.create(newBlog);
      res
        .status(200)
        .json({ data: createResult, message: "Blog Create Successfully!" });
    }
  } catch (error) {
